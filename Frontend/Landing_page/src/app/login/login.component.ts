...
export class LoginComponent {
  ...
  sendOtpToMail() {
    if (this.forgotPasswordForm.valid) {
      ...
      this.http.post('http://localhost:8080/sendmail', otpRequest).subscribe(
        (res: any) => {
          this.sendOtp = false;
        },
        (err) => {
  ...
  resetPassword() {
    if (this.OtpForm.valid) {
      ...
      this.http.post('http://localhost:8080/verifyotp', verifyotp).subscribe(
        (res: any) => {
  ...
  updatePassword() {
    if (this.PasswordForm.valid) {
      ...
      this.http
        .put('http://localhost:8080/user/updatePassword', newPassword)
        .subscribe(
          (res) => {
